...
DECLARE
    ...
    tbl RECORD;
BEGIN
    -- Drop ALL RLS policies on payment tables
    FOR pol IN 
        SELECT schemaname, tablename, policyname 
        FROM pg_policies 
        WHERE schemaname = 'public'
        AND tablename IN ('payments', 'subscriptions', 'invoices', 'daily_summary')
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON %I.%I', 
            pol.policyname, pol.schemaname, pol.tablename);
    END LOOP;

    -- Drop ALL triggers on payment tables
    FOR trig IN 
        SELECT trigger_name, event_object_table 
        FROM information_schema.triggers 
        WHERE trigger_schema = 'public'
        AND event_object_table IN ('payments', 'subscriptions', 'invoices', 'daily_summary')
    LOOP
        BEGIN
    ...
    -- Drop functions
    DROP FUNCTION IF EXISTS update_daily_summary_on_payment() CASCADE;
    DROP FUNCTION IF EXISTS update_daily_summary_on_appointment() CASCADE;
...
SELECT 'Payment schema created successfully' as message;
